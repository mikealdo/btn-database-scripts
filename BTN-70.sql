SELECT *
FROM BTN_DOC d LEFT JOIN BTN_DOC_LINE_NOPART dl ON d.ID = dl.BTN_DOC_ID
WHERE d.DOC_STATUS = 'W' AND dl.LINE_STATUS = 'X';
...
WHERE dl.LINE_STATUS = 'X'
GROUP BY dl.LINE_STATUS, dl.BTN_DOC_ID
HAVING count(*) = 1;
...
FROM (SELECT
      ...
      WHERE this_.DOC_TYPE = ? AND this_.DOC_NO = '300083345'
      ORDER BY this_.DOC_DATE DESC)
WHERE rownum <= 50