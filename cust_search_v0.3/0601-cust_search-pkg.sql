CREATE OR REPLACE PACKAGE BODY "BT_JMSADMIN"."BT_CUST_SEARCH_PKG" AS

  VER_B VARCHAR2(10) := '1.00';

  -- add message to jms queue
  -- NOTE: uses syntax compatible with Oracle XE for development and testing
  PROCEDURE enqueue_cust_search_jms(message_body IN VARCHAR2)
  IS
  ...
  BEGIN
    ...
    sys.DBMS_AQ.ENQUEUE(queue_name => 'BT_JMSADMIN.jms_cust_search_queue',
  ...
  BEGIN
    message_body := '{'
      ...
      || ', "status": "' || (CASE WHEN new_cust_record_type IN ('Z', 'A') THEN new_cust_status ELSE new_place_status END) || '"'
      || ', "custNo": "' || new_cust_no || '"'
  ...
  -- change detection (on update only queue when relevant fields change)
  PROCEDURE has_search_diff_mdcap(
    new_bt_cust_id IN MD_CUST_OWNER.CUST_ADDRESS_PLACE.CUST_ID%TYPE,
    new_place_town IN MD_CUST_OWNER.CUST_ADDRESS_PLACE.TOWN%TYPE,
    new_place_address IN MD_CUST_OWNER.CUST_ADDRESS_PLACE.ADDRESS%TYPE,
    new_place_post_cd IN MD_CUST_OWNER.CUST_ADDRESS_PLACE.POST_CD_ADDR%TYPE,
  ...
  -- change detection (on update only queue when relevant fields change)
  PROCEDURE has_search_diff_btcp(
    new_bt_cust_id IN BT_OWNER.BT_CUST_P.BT_CUST_ID%TYPE,
    new_place_name IN BT_OWNER.BT_CUST_P.NAME%TYPE,
  ...
  -- change detection (on update only queue when relevant fields change)
  PROCEDURE has_search_diff_mdcn(
    new_bt_cust_id IN MD_CUST_OWNER.CUST_NOM.BT_CUST_ID%TYPE,
    new_place_name IN MD_CUST_OWNER.CUST_NOM.NAME%TYPE,
  ...
  -- change detection (on update only queue when relevant fields change)
  PROCEDURE has_search_diff_mdca(
    new_bt_cust_id IN MD_CUST_OWNER.CUST_ACQ.BT_CUST_ID%TYPE,
    new_cust_full_name IN MD_CUST_OWNER.CUST_ACQ.CUST_FULL_NAME%TYPE,
    new_place_name IN MD_CUST_OWNER.CUST_ACQ.NAME%TYPE,
    new_place_town IN MD_CUST_OWNER.CUST_ACQ.TOWN%TYPE,
  ...
  -- change detection (on update only queue when relevant fields change)
  PROCEDURE has_search_diff_btcz(
    new_bt_cust_id IN BT_OWNER.BT_CUST_Z.BT_CUST_ID%TYPE,
    new_cust_full_name IN BT_OWNER.BT_CUST_Z.CUST_FULL_NAME%TYPE,
    ...
    IF (operation != 'ERROR' AND operation != 'DELETE') THEN
      IF (is_equal = FALSE) THEN
        build_jms_msg_btcp(operation,
          ...
          jms_message_body);
        enqueue_cust_search_jms(jms_message_body);
        -- when creating a workplace, it is necessary to also sync fields derived from
        -- parent / hq entities i.e. BT_CUST_Z and MD_CUST_OWNER.CUST
        IF (operation = 'CREATE') THEN
          -- find and apply fields from Z hq
          FOR custz IN (SELECT z.cust_full_name FROM BT_OWNER.BT_CUST_Z z WHERE z.store_no=new_store_no AND z.cust_no=new_cust_no)
          LOOP
            build_jms_msg_btcz(operation,
              new_bt_cust_id, custz.cust_full_name,
              jms_message_body);
          enqueue_cust_search_jms(jms_message_body);
          END LOOP;
          -- find and apply fields from MD_CUST_OWNER.CUST hq
          FOR cust IN (SELECT c.cust_reg_no FROM MD_CUST_OWNER.CUST c WHERE c.store_no=new_store_no AND c.cust_no=new_cust_no)
          LOOP
            build_jms_msg_mdc(operation,
              new_bt_cust_id,
              cust.cust_reg_no,
